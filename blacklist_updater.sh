...
#                Автоматически настраивает cron и права на каталог.
# Автор:        blablajka
# Версия:       3.0 (Debian-ready)
# ============================================================
...
# --- 1. Настройка прав на каталог (без ошибок invalid user) ---
setup_permissions() {
    mkdir -p "$BLACKLIST_DIR"
    # Проверяем, существует ли пользователь syslog
    if id "syslog" &>/dev/null; then
        chown syslog:adm "$BLACKLIST_DIR"
    else
        chown root:adm "$BLACKLIST_DIR"
    fi
    chmod 0755 "$BLACKLIST_DIR"
}

# --- 2. Автоматическая настройка cron ---
setup_cron() {
...
# Если запущены от root, выполняем подготовку
if [ "$EUID" -eq 0 ]; then
    setup_permissions
    setup_cron
fi
